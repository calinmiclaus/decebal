...
/* This function tests if there is a file system of a particular type
 * stored in a region.  If it does not find any file system, it returns
 * NULL.  If it does find one, it returns the exact region the file
 * system occupies.
 */
PedGeometry*
...
/* This function tries to figure out what file system type (if any) is stored
 * in a particular region.  It tries to be clever at dealing with ambiguous
 ...
 * file system occupies a given region.
 */
int
...
/* This function opens the file system stored inside a region, if it
 * can find one.
 */
PedFileSystem*
...
/* This function initializes a new file system on a region, writing
 * out appropriate metadata and signatures.
 */
PedFileSystem*
 ...
 * of a particular type created with ped_file_system_create() must satisfy.
...
/* This function returns a constraint on the regions that fs can be
 * resized to.  This takes into account the amount of used space on
 * the filesystem and the capabilities of the resize algorithm.
 */
PedConstraint*
